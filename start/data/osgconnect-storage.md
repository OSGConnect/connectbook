...
**USERS SHOULD NEVER SUBMIT JOBS FROM WITHIN**, and should not list `/public` files in the `transfer_input_files` line of a submit file, unless as an HTTP address (see more details below). Files placed in `/public` should only be accessed by jobs using the below tools (see "Transferring Data To/From Jobs", below). Users violating these policies may lose the ability to submit jobs until their submissions are corrected.
...
| **OSG's StashCache** | >1 GB per file; <50 GB per job | `/public` | `stashcp` | For the largest input files, staged within `/public`| [StashCache](https://support.opensciencegrid.org/support/solutions/articles/12000002775)|
